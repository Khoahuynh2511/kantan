...
export function logError(
  ...
  try {
    const savedErrors = localStorage.getItem('kantan_error_logs');
    ...
    localStorage.setItem('kantan_error_logs', JSON.stringify(trimmedLogs));
  } catch (storageError) {
...
export function clearErrorLogs(): void {
  try {
    localStorage.removeItem('kantan_error_logs');
  } catch (error) {
